        ...
        actions={
          <div className="flex items-center gap-2">
            {props.openApiDoc.exists ? (
              <button
                disabled={props.busy}
                onClick={props.deleteOpenApiDoc}
                className="rounded-xl border border-rose-700 px-4 py-2 text-sm font-medium text-rose-300 hover:bg-rose-900/30 disabled:opacity-50"
              >
                Delete
              </button>
            ) : null}
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium disabled:opacity-50"
            >
      ...
      >
        ...
        {!props.openApiDoc.exists ? (
          <div className="rounded-xl border border-dashed border-zinc-700 p-6 text-sm text-zinc-400">
            No OpenAPI contract uploaded yet.
          </div>
        ) : (
          <div>
            <div className="mb-3 flex items-center justify-between">
              <p className="text-xs text-zinc-400">
                Loaded format:{" "}
                <span className="text-zinc-200">{props.openApiDoc.format}</span>
              </p>
              <div className="inline-flex rounded-lg border border-zinc-700 overflow-hidden">
                <button
                  onClick={() => setViewMode("rendered")}
                  className={`px-3 py-1.5 text-xs ${viewMode === "rendered" ? "bg-zinc-700 text-zinc-100" : "bg-zinc-900 text-zinc-400"}`}
                >
                  Rendered Docs
                </button>
                <button
                  onClick={() => setViewMode("raw")}
                  className={`px-3 py-1.5 text-xs ${viewMode === "raw" ? "bg-zinc-700 text-zinc-100" : "bg-zinc-900 text-zinc-400"}`}
                >
                  Raw Contract
                </button>
              </div>
            ...
            {viewMode === "rendered" ? (
              parsedSpec ? (
                ...
                <div className="rounded-xl border border-dashed border-zinc-700 p-6 text-sm text-zinc-400">
                  Could not parse OpenAPI contract for rendering.
                </div>
              )
            ) : (
